...
if ( $_SERVER["REQUEST_METHOD"] == "POST" and isset($_POST["pay_button"]) ):
  ...
  " INSERT INTO payments (
    payment_status, payment_action_date, payment_platform_transfer_number,
    payment_transfer_number, payment_operation_number, payment_total_paid_amount, user_id
  )
  VALUES (
    2, '$date', '$platform_transfer_number',
    '$transfer_number', '$operation_number', $total_amount, $user_id
  )";
  $sql->query($query);
  ...
  foreach ($cart_courses as $key => $cart_course) {
    ...
    $query =
    " INSERT INTO payment_courses (payment_id, course_id, course_name, course_price)
      VALUES ($payment_id, $course_id, '$course_name', $course_price)
    ";
    $sql->query($query);
  }
  ...
  upload_file($operation_image, "operation_image", "payments", $payment_id, true);
